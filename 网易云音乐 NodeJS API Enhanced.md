...
1. 点击`完成`,等待部署完成,点击`资源列表`的 `API网关` 里的 `URL`,正常情况会打开文档地址,点击文档`例子`可查看接口调用效果
...
**必选参数 :** `id` : 音乐 id `level`: 播放音质等级, 分为 `standard` => `标准`,`higher` => `较高`, `exhigh`=>`极高`, `lossless`=>`无损`, `hires`=>`Hi-Res`, `jyeffect` => `高清环绕声`, `sky` => `沉浸环绕声`, `dolby` => `杜比全景声`, `jymaster` => `超清母带` `unblock`: 是否使用`UnblockNeteaseMusic`, 分为`true`和`false`
...
说明: 可以上传声音到播客,例子在 `/public/voice_upload.html` 访问地址: [/voice_upload.html](https://pkllzbbagoeg.ap-southeast-1.clawcloudrun.com/voice_upload.html)
